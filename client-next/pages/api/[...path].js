...
module.exports = (req, res) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        // const isLogin = pathname === '/api/v1/login'
				...
				const pathname = req.query.path.join('/'); // e.g. req.query.path = ['v1', 'follow']
				const isLogin = pathname === 'v1/login';
        ...
        if (authToken) {
            req.headers['jwtToken'] = authToken
        }
        ...
        function interceptLoginResponse(proxyRes, req, res) {
            ...
            proxyRes.on('end', () => {
                try {
                    ...
                    const { jwtToken, id } = JSON.parse(apiResponseBody)
